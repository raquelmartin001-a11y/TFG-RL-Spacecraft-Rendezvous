...
def propagate_HCW(r0, v0, dt, omega):
    ...
    y = 6.*(sin(omega*dt) - omega*dt)*x0 + y0 + 2.*((cos(omega*dt) - 1.)/omega)*xdot0 + (4.*sin(omega*dt)/omega - 3.*dt)*ydot0
    z = cos(omega*dt)*z0 + (sin(omega*dt)/omega)*zdot0
    r = np.array([x, y, z])

    # Final velocity vector
    xdot = 3.*omega*sin(omega*dt)*x0 + cos(omega*dt)*xdot0 + 2.*sin(omega*dt)*ydot0
    ydot = 6*omega*(cos(omega*dt) - 1.)*x0 - 2.*sin(omega*dt)*xdot0 + (4.*cos(omega*dt) - 3.)*ydot0
