...
class RendezvousEnv(gym.Env):
    ...
    def __init__(self, obs_type, \
        ...
        self.rTf = np.array(rTf, dtype=np.float64)
        self.vTf = np.array(vTf, dtype=np.float64)
        self.dr0_max = np.array(dr0_max)
        self.dv0_max = np.array(dv0_max)
        ...
        if self.obs_type == 0:
            ...
            x_ub = np.array([+self.max_r, +self.max_r, +self.max_r, \
                +self.max_v, +self.max_v, +self.max_v, \
                self.NSTEPS, 1., 1., 1.], dtype=np.float32)
        
        self.observation_space = spaces.Box(x_lb, x_ub, dtype=np.float32)
        ...
        self.action_space = spaces.Box(a_lb, a_ub, dtype=np.float32)
        
        """ Environment initialization """
        self.viewer = None
        self.state = None
    
    """ Set seed — handled via reset(seed=...) in Gymnasium """
    # NOTE: seed() is deprecated in Gymnasium; seeding is now done in reset()
    ...
    def operConstraints(self, rk):
        ...
        if (r_chaser < self.rKOZ):
            ...
            dis1 = -x + y*np.tan(self.beta_cone)
            dis2 = x + y*np.tan(self.beta_cone)
            dis3 = z + y*np.tan(self.beta_cone)
            dis4 = -z + y*np.tan(self.beta_cone)
    ...
    def ICvariation(self):
        ...
        dv0 = np.array([dxdot0, dydot0, dzdot0])
    ...
    def perturbAction(self, action):
        ...
        #Rotation matrix
        Arot = np.array([[1., -du_rot[2], du_rot[1]], 
                  [du_rot[2], 1., -du_rot[0]], 
                  [-du_rot[1], du_rot[0], 1.]])
    ...
    def eval_observations(self, rk_obs, vk_obs, tk_left, action):
        ...
        if self.obs_type == 0:
            obs = np.array([rk_obs[0]/self.rconv, rk_obs[1]/self.rconv, rk_obs[2]/self.rconv, \
                ...
                tk_left]).astype(np.float32)
        else:
            obs = np.array([rk_obs[0]/self.rconv, rk_obs[1]/self.rconv, rk_obs[2]/self.rconv, \
                vk_obs[0]/self.vconv, vk_obs[1]/self.vconv, vk_obs[2]/self.vconv, \
                tk_left, action[0], action[1], action[2]]).astype(np.float32)
    ...
    def step(self, action):
        ...
        if self.stochastic == True:
            ...
            action = min(np.sqrt(3.), norm(action))*action/norm(action)
        ...
        terminated = done   # episode ended due to logic (goal reached or constraint violated)
        truncated = False   # episode ended due to time limit (handled inside isDone)
    ...
    def reset(self, seed=None, options=None):
        ...
        :return info: empty info dict (required by Gymnasium API)
        """
        # Gymnasium: handle seeding in reset()
